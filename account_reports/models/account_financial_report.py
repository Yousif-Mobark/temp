...
class AccountFinancialReportLine(models.Model):
    ...
    def _format(self, value):
        ...
        if self.figure_type == 'float':
            ...
            #Mudathir:if greater than zero show it as is else set brakets and convert number to positive with abs()
            if value['name'] >= 0:
                value['name'] = formatLang(self.env, value['name'], currency_obj=currency_id)
            else:
                value['name'] = formatLang(self.env, abs(value['name']), currency_obj=currency_id)
    ...
    def get_lines(self, financial_report, currency_table, options, linesDicts):
        ...
        for line in self:
            ...
            if len(lines) == 1:
                ...
                result = lines
            print("final_result_table>>>>>>>>>>>>>>>>>> >>>>>",result)
