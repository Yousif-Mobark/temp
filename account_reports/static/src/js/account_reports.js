...
var accountReportsWidget = Widget.extend(ControlPanelMixin, {
    ...
    render_footnotes: function() {
        ...
        console.log("Render footnote template")
        return this._rpc({
                model: this.report_model,
    ...
    fold: function(line) {
        ...
        var line_parent_id = line.data('parent-id');
        console.log("FoldParent>>>>>>>>>",line_parent_id)
        if (line_parent_id == null){
        line_parent_id = line_id;
        }
        console.log("FoldnewParent>>>>>>>>>",line_parent_id)
        var level = line.data('level')+1;
        console.log("Fold>>>>>>>>>",line_id)
        console.log("level>>>>>>>>>",level)
        line.find('.fa-caret-down').toggleClass('fa-caret-right fa-caret-down');
        line.toggleClass('folded');
        var $lines_to_hide = this.$el.find('tr[data-level="'+level+'"][data-parent-id="'+line_parent_id+'"]');
        console.log("Lines>>>>>>>>>",$lines_to_hide)
    ...
    unfold: function(line) {
        var self = this;
        var line_id = line.data('id');
        console.log("UNFold_id>>>>>>>>>",line_id)
        var line_parent_id = line.data('parent-id');
        console.log("parent_id>>>>>>>>>",line_parent_id)
        if (line_parent_id == null){
        line_parent_id = line_id;
        }
        var level = line.data('level');
        console.log("level1>>>>>>>>>",level)
        level= level + 1
        console.log("level2>>>>>>>>>",level)
        line.toggleClass('folded');
        self.report_options.unfolded_lines.push(line_id);
        var $lines_in_dom = this.$el.find('tr[data-level="'+level+'"][data-parent-id="'+line_parent_id+'"]');
        console.log("UNFoldLines>>>>>>>>>",$lines_in_dom)
        if ($lines_in_dom.length > 0) {
            console.log("unfoldLenses>>>>>>>>>",$lines_in_dom.length)
        ...
        console.log("unfoldRPC>>>>>>>>>",$lines_in_dom.length)
            return this._rpc({
                    model: this.report_model,
                    method: 'get_html',
                ...
                .then(function(result){
                   ...
                   var child = $(line).find('[data-id="'+line_id+'"]');
                   if (child) {
                    self.fold(child);
                    }
